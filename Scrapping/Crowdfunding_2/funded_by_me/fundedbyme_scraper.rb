...
def login(email, password)
  ...
  # @driver.div(class: 'VfPpkd-dgl2Hf-ppHlrf-sM5MNb').click
  email = 'testinguser1624@gmail.com'
  @driver.input(id: 'id_username').send_keys email.to_s
  password = 'qwerty'
  @driver.input(id: 'id_password').send_keys password.to_s
  @driver.input(class: ["button", "submit-btn", "mt0"]).click
  byebug
  sleep 5
end
  ...
  begin
    ...
    init_page = 0
      loop do
        ...
        if jsonres['results'] != '' || !jsonres['results'].nil?
...
def get_individual_company_details(status, company_url, csv)
  ...
  begin
    ...
    captured_date = DateTime.now.strftime('%d/%m/%Y') rescue ''
    source = 'FUNDEDBYME'
